...
module.exports.write = () => {
  return (req, res, next) => {
    ...
    try {
      ...
      res.status(500).send(new HttpError(500, 'Internal Server Error', e))
    }
  }
}

module.exports.errors = mappings => {
  let mappingFn
  ...
  return (err, req, res, next) => {
    ...
    res.status(err.code).set('content-type', 'application/json').send(JSON.stringify(err))
  }
}
