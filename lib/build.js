...
module.exports = (options = {}) => {
  ...
  const watchRemove = filename => {
    ...
    Promise.resolve(cache).then(buildRegistry).then(boundWriteRegistry).catch(console.error)
  }

  initPromise
    .then(() => {
      console.log(`Registry built. Watching files...`)
      chokidar.watch(glob, WATCH_OPTIONS).on('add', watchUpdate).on('change', watchUpdate).on('unlink', watchRemove).on('error', console.error)
    })
    .catch(console.error)
}
...
module.exports.getRegistryFromGlob = (glob, root = process.cwd()) => {
  const boundImportFiles = importFiles.bind(null, root)
  return fastGlob(glob, GLOB_OPTIONS).then(boundImportFiles).then(extractEndpoints).then(buildRegistry)
}
