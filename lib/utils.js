...
module.exports.fromUrlEncoded = urlencoded => {
  const result = {}
...
module.exports.normalizeHeaders = headers => {
  let newHeaders = {}
...
const HttpError = (module.exports.HttpError = function (...params) {
  ...
  if (params[0] instanceof Error || params[0] instanceof AppError) {
    // (err[, inner])
    this.code = 500
    ...
    // (code[, message[, detail/inner[, inner]]])
    this.code = params[0]
    this.type = HTTP_STATUS_MESSAGES[this.code] || 'Unknown Http Status'
    this.name = 'HttpError'
